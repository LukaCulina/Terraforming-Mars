...
public class HostGameStateCoordinator implements GameStateListener {
    ...
    private void handleJoiningPhase(GameState state) {
        ...
        if (allJoined) {
            log.info("All players have joined, transitioning to Corporation Selection");
    ...
    private void handleCorporationSelection(GameState state) {
        ...
        if (allChosen) {
            log.info("All players have chosen corporations, transitioning to Card Draft");
    ...
    private void handlePlayingPhase(GameState state) {
        ...
        if (controller != null) {
            ...
            log.warn("Received Playing phase state, but controller is not set!");
        }
    }
}